...
# Test###
trial_design <- trial_design[1:2, ] # test sur les deux premiers scénarios plutôt que suir les 1296
trial_design$N <- 2 # pour chaque scénario, seulement 2 répétitions du combo "génération + clustering"
trial_design$nstarts <- 1  # Réduit le nombre d'initialisations
...
output <- parallelLapply(1:2, fun = par_fun) # appplique par_fun seulement aux deux premières lignes de trial_design
###
parallelStop()

results_all <- lapply(output, function(x) x[[1]])
#Test###
saveRDS(output, "output_test.rds") #enregistre les résultats du test
...
### Résultats Test###
output_test <- readRDS("output_test.rds")
str(output_test, max.level = 2)

for (i in seq_along(output_test)) {
  cat("\n--- Résultat scénario", i, "---\n")
  print(output_test[[i]][[1]])  # Paramètres
}
